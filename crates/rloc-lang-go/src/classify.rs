    ...
    while index < bytes.len() {
        match *state {
            ScannerState::Code => {
                ...
                if bytes[index] == b'\'' {
                    has_code = true;
                    *state = ScannerState::Rune { escaped: false };
                    index += 1;
                    continue;
                }
            ...
            ScannerState::Rune { escaped } => {
                has_code = true;
                match bytes[index] {
                    b'\\' if !escaped => {
                        *state = ScannerState::Rune { escaped: true };
                        index += 1;
                    }
                    b'\'' if !escaped => {
                        *state = ScannerState::Code;
...
fn carried_code(state: ScannerState) -> bool {
    matches!(
        state,
        ScannerState::String { .. } | ScannerState::RawString | ScannerState::Rune { .. }
    )
}
...
fn line_reason(raw_kind: LineKind, effective_kind: LineKind) -> &'static str {
    match (raw_kind, effective_kind) {
        (LineKind::Mixed, LineKind::Comment) => {
            "line contains Go code and comment segments, but classification policy excludes mixed lines from code"
        }
...
fn line_reason_for_kind(kind: LineKind) -> &'static str {
    match kind {
        ...
        LineKind::Code => {
            "line contains Go code, raw string content, or quoted literals without comments"
        }
        LineKind::Comment => "line is part of a Go comment",
        LineKind::Mixed => "line contains Go code and comment segments",
    }
}
